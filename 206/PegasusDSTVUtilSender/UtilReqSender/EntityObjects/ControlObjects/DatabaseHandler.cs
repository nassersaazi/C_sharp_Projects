...
namespace UtilReqSender.ControlObjects
{
    public class DatabaseHandler
    {
        ...
        internal DataTable GetMtnDstvTransactionsToSend()
        {
            DataTable datatable = new DataTable();
            try
            {
                command = PegPayInterface.GetStoredProcCommand("GetMtnDstvTransactionsToSend");
                datatable = PegPayInterface.ExecuteDataSet(command).Tables[0];
            }
            catch (Exception ex)
            {
                return GetMtnDstvTransactionsToSend();
            }
            return datatable;
        }

        internal DataTable GetMtnDstvTransactionsToSend_1()
        {
            DataTable datatable = new DataTable();
            try
            {
                command = PegPayInterface.GetStoredProcCommand("GetMtnDstvTransactionsToSend_1");
                datatable = PegPayInterface.ExecuteDataSet(command).Tables[0];
            }
            catch (Exception ex)
            {
                LogError(ex.Message, "MTN", DateTime.Now, "DSTV");
                return GetMtnDstvTransactionsToSend();
            }
            return datatable;
        }
