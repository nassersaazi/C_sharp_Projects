...
public class TotalLogic
{
    ...
    public TotalCardValidationResponse ValidateCard(string cardNo)
    {
        ...
        try
        {
            response = CardValidate(cardNo);
            if (response.StatusDescription == "SUCCESS")
            {
                response = GetCardHolderName(cardNo);
            }
        }
        catch (Exception exc)
        {
            response.StatusCode = "100";
            response.StatusDescription = "An error occured on validating the Card";
        }
        return response;
    }

    public TotalCardValidationResponse GetCardHolderName(string cardNo)
    {
        TotalCardValidationResponse response = new TotalCardValidationResponse();
        ...
        SOAPReqBody.LoadXml("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:tem=\"http://tempuri.org/\" xmlns:efu=\"http://schemas.datacontract.org/2004/07/EfuelWebServices.Contracts\">" +
                               "<soapenv:Header/>" +
                                ...
                                "</soapenv:Body>" +
                            "</soapenv:Envelope>");
        string xmlRequest = "Request to Total at " + DateTime.Now + Environment.NewLine + SOAPReqBody.OuterXml;
        try
        {
            ...
            using (WebResponse Serviceres = Req.GetResponse())
            {
                using (StreamReader rd = new StreamReader(Serviceres.GetResponseStream()))
                {
                    ...
                    xmlResponse = "Response from Total at " + DateTime.Now + Environment.NewLine + xmlString;
                }
            }
            ...
            {
                response.StatusCode = "100";
                response.StatusDescription = "An error occured on getting card details";
                dh.LogTotalError(cardNo, "", "GetCardInformation: " + ex.Message);
            }
        }
        catch (WebException webex)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    xmlResponse = "Response from Total at " + DateTime.Now + Environment.NewLine + feedback;
                }
                XmlDocument doc = new XmlDocument();
            ...
            catch (Exception ex)
            {
                ...
                response.StatusDescription = "Unable to connect to Total at the moment";
                dh.LogTotalError(cardNo, "", "GetCardInformation: " + webex.Message);
            }
        }
        catch (Exception exc)
        {
            ...
            response.StatusDescription = "An error occured on getting card details";
            dh.LogTotalError(cardNo, "", "GetCardInformation: " + exc.Message);
        }
        dh.LogRequestResponse(cardNo, "GetCardInformation", xmlRequest, xmlResponse);
        return response;
    }

    public TotalCardValidationResponse CardValidate(string cardNo)
    {
        TotalCardValidationResponse response = new TotalCardValidationResponse();
        string xmlString, xmlResponse = "";
        XmlDocument SOAPReqBody = new XmlDocument();
        SOAPReqBody.LoadXml(@"<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:tem=""http://tempuri.org/"" xmlns:efu=""http://schemas.datacontract.org/2004/07/EfuelWebServices.Contracts"">
                                   ...
                                   <soapenv:Body>
                                      <tem:SetCardCreditEnhanced>
                                         <tem:cardCredit>
                                            <efu:CardID>"+ cardNo + @"</efu:CardID>
                                            <!--Optional:-->
                                            <efu:Amount>500</efu:Amount>
                                            <efu:Currency>CFA</efu:Currency>
                                            <!--Optional:-->
                                            <efu:CreditOrderOption>Delayed</efu:CreditOrderOption>
                                         </tem:cardCredit>
                                      </tem:SetCardCreditEnhanced>
                                   </soapenv:Body>
                                </soapenv:Envelope>");
        ...
        {
            HttpWebRequest Req = (HttpWebRequest)WebRequest.Create(Url);
            Req.Headers.Add(@"SOAPAction:http://tempuri.org/IServiceEfuel/SetCardCreditEnhanced");
            Req.ContentType = "text/xml;charset=\"utf-8\"";
            Req.Accept = "text/xml";
            Req.Method = "POST";
            X509Certificate2 cert = new X509Certificate2(certPath, certPassword, X509KeyStorageFlags.MachineKeySet);
            ...
            try
            {
                XmlNodeList _credit_order_id = doc.GetElementsByTagName("a:CreditOrderID");
                XmlNodeList _status_code = doc.GetElementsByTagName("a:ReturnCode");

                response.StatusCode = _status_code[0].InnerText.ToString();
                //response.CreditOrderId = _credit_order_id[0].InnerText.ToString();
                response.StatusDescription = "SUCCESS";
            ...
            {
                response.StatusCode = "100";
                response.StatusDescription = "An error occured on validating a card";
                dh.LogTotalError(cardNo, "", "GetCreditOrderId - Validation: " + ex.Message);
            }
        }
        catch (WebException webex)
        {
            ...
            try
            {
                ...
                response.StatusCode = _status_code[0].InnerText.ToString();
                response.StatusDescription = GetErrorDescription(response.StatusCode); ;
                dh.LogTotalError(cardNo, "", "GetCreditOrderId - Validation: " + webex.Message);
        ...
        catch (Exception exc)
        {
            ...
            //response.StatusDescription = exc.Message;
            response.StatusDescription = "An error occured on validating a card";
            dh.LogTotalError(cardNo, "", "GetCreditOrderId - Validation: " + exc.Message);
        }
        dh.LogRequestResponse(cardNo, "GetCreditOrderId - Validation", xmlRequest, xmlResponse);
        return response;
    }
