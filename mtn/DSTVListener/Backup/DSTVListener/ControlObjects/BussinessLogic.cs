    ...
    public class BussinessLogic
    {
        ...
        private string QueryForBoxOfficeCustomer(GetFinancialInformationRequest GetFriRequest)
        {
            ...
            catch (Exception e)
            {
                ...
                dh.LogError("QueryDetails: " + e.Message);
            }
            return xml;
        ...
        private string GetFRIResponeXmlForBoxOffice(DSTVListener.UtilitiesApi.Response queryResponse, GetFinancialInformationRequest GetFriRequest)
        {
            ...
            if (queryResponse.ResponseField6.Equals("0"))
            {
                xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
                      "<ns2:getfinancialresourceinformationresponse xmlns:ns2=\"http://www.ericsson.com/em/emm/serviceprovider/backend/client\">" +
        ...
        {
            string xml = "xml = \"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
                         "<ns2:getfinancialresourceinformationresponse xmlns:ns2=\"http://www.ericsson.com/em/emm/serviceprovider/backend/client\">" +
        ...
        private string GetAmountDue(string bouquetPrice, string customerBal)
        {
            ...
            try
            {
                ...
                int bouqPrice = Convert.ToInt32(bouquetPrice.Split('.')[0]);
                int customerBalance = Convert.ToInt32(customerBal.Split('.')[0]);
                int result = bouqPrice + customerBalance;
                if (result < 0)
                {
