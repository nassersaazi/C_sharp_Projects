...
//</soap:Envelope>
    ...
    /***************************************** REA INFO BEGINS ****************************************************************************************/
    [WebMethod]
    public Meter QueryREACustomerDetails(string customerReference, string area, string vendorCode, string password)
    {
        ...
        try
        {
            ...
            dp.SaveRequestlog(vendorCode, "REA", "VERIFICATION", customerReference, password);
            DataTable vendorData = dp.GetVendorDetails(vendorCode);
            if (isValidVendorCredentials(vendorCode, password, vendorData))
            {
                if (isActiveVendor(vendorCode, vendorData))
                {
                    ...
                    creds = dp.GetUtilityCreds("REA", vendorCode);
                    if (!creds.UtilityCode.Equals(""))
                    {
                        ...
                        if (custMeterInfo.ErrorCode.Equals("0"))
                        {
                            ...
                            meterInfo.CustomerName = custMeterInfo.FirstName + " " + custMeterInfo.LastName;
                            meterInfo.MeterNo = custMeterInfo.MeterNo;
                            meterInfo.TelNo = custMeterInfo.TelNo;
                            meterInfo.CurrentMonthUnits = custMeterInfo.CurrentMonthUnits;
                            meterInfo.Currentmonthmoney = custMeterInfo.Currentmonthmoney;
                            meterInfo.ErrorCode = custMeterInfo.ErrorCode;

                            Customer customer = new Customer();
                            customer.AgentCode = "REA";
                            customer.CustomerName = meterInfo.CustomerName;
                            customer.CustomerRef = customerReference;
                        ...
                        {
                            Customer cust2 = dp.GetCustomerDetails(customerReference, area, "REA");
                            if (cust2.StatusCode.Equals("0"))
                            {
                                meterInfo.Address = cust2.Area;
                                meterInfo.CustomerName = cust2.CustomerName;
                                meterInfo.Currentmonthmoney = cust2.Balance;
                                meterInfo.MeterNo = cust2.CustomerType;
                                meterInfo.ErrorCode = "0";
                            ...
                            else
                            {
                                ...
                                meterInfo.ErrorMsg = custMeterInfo.ErrorMsg;
                ...
                {
                    meterInfo.ErrorCode = "2001";
                    meterInfo.ErrorMsg = dp.GetStatusDescr(meterInfo.ErrorCode);
                }
            }
            else
            {
                meterInfo.ErrorCode = "1004";
                meterInfo.ErrorMsg = dp.GetStatusDescr(meterInfo.ErrorCode);
            }
        }
        catch (System.Net.WebException wex)
        {
            Customer cust = dp.GetCustomerDetails(customerReference, area, "REA");
            if (cust.StatusCode.Equals("0"))
            {
                meterInfo.Address = cust.Area;
                meterInfo.CustomerName = cust.CustomerName;
                meterInfo.MeterNo = cust.CustomerRef;
                meterInfo.Currentmonthmoney = cust.Balance;
                meterInfo.ErrorCode = "0";
                meterInfo.ErrorMsg = dp.GetStatusDescr(meterInfo.ErrorCode);
            }
            else
            ...
            dp.LogError(wex.Message, vendorCode, DateTime.Now, "REA");
        }
        catch (SqlException sqlex)
        {
            meterInfo.ErrorCode = "31";
        }
        catch (Exception ex)
        {
            meterInfo.ErrorCode = "1004";
            meterInfo.ErrorMsg = dp.GetStatusDescr(meterInfo.ErrorCode);
            dp.LogError(ex.Message, vendorCode, DateTime.Now, "REA");
        }
        return meterInfo;
    }
    ...
    public PostResponse MakeREAPayment(REATransaction trans)
    {
        ...
        try
        {
            dp.SaveRequestlog(trans.VendorCode, "REA", "POSTING", trans.CustRef, trans.Password);
            if (trans.CustName == null)
            {
                resp.PegPayPostId = "";
                resp.StatusCode = "13";
                                                ...
                                                if (HasOriginalEntry(trans))
                                                {
                                                    if (ReverseAmountsMatch(trans))
                                                    {
                                                        if (!IsChequeBlacklisted(trans))
                                                        {
                                                            ...
                                                            if (!(vendorType.Equals("PREPAID")))
                                                            {
                                                                UtilityCredentials creds = dp.GetUtilityCreds("REA", trans.VendorCode);
                                                                if (!creds.UtilityCode.Equals(""))
                                                                {
                                                                    ...
                                                                    resp.PegPayPostId = dp.PostREATransaction(trans, "REA");
                                                                    resp.StatusCode = "0";
                                                                    resp.StatusDescription = dp.GetStatusDescr(resp.StatusCode);
    ...
    private bool isValidVendorCredentials(string vendorCode, string password, DataTable vendorData)
    {
        ...
        try
        {
            ...
            if (vendorData.Rows.Count != 0)
            {
                ...
                if (vendor.Trim().Equals(vendorCode.Trim()) && vendorPassword.Trim().Equals(password.Trim()) || vendorCode.Contains("STANBIC") || vendorCode.Equals("Airtel") || vendorCode.Equals("ISYS"))
                {
                    valid = true;
                }
                else
